...
| カウンター | サイドパネルの装飾 | 等幅フォント、実アクセス数と誤認させない表示 |
...
### トップページのデスクトップ構成### トップページのデスクトップ構成
...
- トップページは縦スクロールを維持するが、各セクションをウィンドウパネルとして連続配置する
- サイドバーはデスクトップで上部追従可能とする。ただしフッター領域へ重ならないよう`position: sticky`の範囲を親コンテナ内に限定する
- サブページでは左サイドバーを共通表示し、メインカラムにページ固有コンテンツを表示する
- フォームページでは入力幅を確保するため、メインカラムを広めに使用する
...
- デフォルトでは実アクセス数を表示しない
- 数字表示を行う場合は、`data/siteConfig.ts`で管理する装飾値またはローカル訪問回数であることを仕様・実装上明示する
- サーバー保存を追加しない限り、全ユーザー共通の累計アクセス数として扱わない
...
- `/public/logo/yukimi_works_favicon.svg`を使用する
...
1. ページ表示直後は吹き出しを表示しない
2. 初回のみ約5秒待機する
3. 候補から1件をランダム表示する
4. 吹き出しは約4秒表示する
5. 消えた後、10〜18秒のランダムな待機時間を空ける
6. 以後同じ処理を繰り返す
...
- 約2.5秒後に`/character/default.png`へ戻す
- 表情変更中に再クリックされた場合は復帰タイマーをリセットし、最新クリックから2.5秒後に戻す
- クリック時のキャラクター移動は横方向、回転、揺れを禁止する。クリック直後のみ`translateY(2px〜3px)`で約100〜150ms下へ沈ませ、表情画像の表示時間とは関係なく即座に元の位置へ戻す

#### 特殊メッセージ抽選

クリックごとに一つの乱数を生成し、排他的に判定する。
...
## Section 3: Philosophy## Section 3: Philosophy（理念）

**ID**: `#philosophy`

### レイアウト

- `Philosophy`タイトルバーを持つウィンドウパネルとして表示する
- 大きな広告風タイポグラフィではなく、短いキャッチコピーと本文を読みやすくまとめる
- 中央揃えを基本とするが、長文は左揃えにして可読性を優先する
...
YukimiWorksは常にユーザー目線でサービス・コンテンツを制作する組織です。
ユーザーが「欲しい」と思えるコンテンツを目指して日々開発・創作を行っています。

見た目の華やかさや一時的な流行だけに捉われず、
使いやすさや心地よさを重視します。

本当に人々にとって使いやすく便利なものを
「創り」「残し」「守っていくこと」
それが私たちの使命です。
```
...
- 「欲しい」を主アクセント色で強調する
- キャッチコピー下に短い青系点線を配置する
- 背景は`--site-surface-soft`、本文領域は白を使用する
- 雪や星の装飾はタイトルバー付近に限定する

### アニメーション

- 初回表示時にパネル全体を短くフェードインする
- 行ごとの大きなスライドアニメーションは使用しない

## Section 4: Pickup Works（ルートページ内）

**ID**: `#pickup-works`

### 概要

`/data/works.ts`の先頭3件、または`featured: true`の項目を表示する。表示順は配列順とし、ソートしない。

### レイアウト

- タイトルバー: `Pickup Works`
- 分類導線の`Works`パネルと区別する
- デスクトップ: 2〜3列
- タブレット: 2列
- モバイル: 1列
- パネル下部に「すべての制作実績を見る」リンクを配置し、`/works`へ遷移する
...
- 白背景、青系1pxボーダー、内側の薄い線で二重線風にする
- サムネイルと本文の間を点線で区切る
- 角丸は最大4px
- カード上部のピンク・パープルグラデーションラインは廃止する
- ホバー時は2px上に移動し、右下に短いピクセル影を表示する
- カード全体を`next/link`でラップする
- フォーカス時は主アクセント色の2pxアウトラインを表示する
- タグは小さな角丸なしのラベル形式とする

## Section 5: News プレビュー（ルートページ内）

**ID**: `#news`

### 概要

`/data/news.ts`の先頭3件、または`featured: true`の項目を表示する。表示順は配列順とし、ソートしない。

### レイアウト

- タイトルバー: `What's New`または`News`
- `Works / News`のような結合タイトルは使用しない
- 日付、カテゴリー、タイトルを一覧形式で表示する
- サムネイルを使用する場合も、カード高さを過度に大きくしない
- 各項目を点線で区切る
- 下部に「すべてのお知らせを見る」リンクを配置し、`/news`へ遷移する

### NewsCardデザイン

- WorkCardと同じ二重線風パネルを使用する
- 日付は等幅フォントで表示する
- カテゴリーは小さな青系ラベルにする
- カード上部グラデーションライン、グラスモーフィズム、強いグローは使用しない
- カード全体をクリック対象とし、`/news/[id]`へ遷移する

## Section 6: Link / Contact（リンク・お問い合わせ）
...
- X、Instagram、Discord、外部サイトなどをアイコン・サービス名・短い説明で表示する
- SNSアイコンは通常、昼食、お菓子の3セットを用意できる
- 外部リンクには`target="_blank" rel="noopener noreferrer"`を設定する
- アイコンのみで意味を伝えず、必ずサービス名をテキスト表示する
- パネル下部に「すべてのリンクを見る」導線を置き、`/links`へ遷移する
- トップ用とリンク一覧用に別データを持たず、表示フラグだけで制御する
...
仕事のご依頼、アプリ・サービスに関するお問い合わせを受け付けています。

[お問い合わせページへ]
```

- `/contact`へのパネル型ボタンを配置する
- CTAは主アクセント色の下線付きリンクまたは小さなレトロボタンとする
- 大きなグラデーションボタンは使用しない

---

## 時間帯テーマ・ランダムイベント仕様## 時間帯テーマ・ランダムイベント仕様
...
| 時間帯 | テーマ名 | 表現 |
|---|---|---|
| 05:00〜06:59 | `early-morning` | 薄いピンク、青みのある影、柔らかい朝焼け |
| 07:00〜10:59 | `morning` | 明るい水色、白、淡い青 |
| 11:00〜16:59 | `day` | 基本のアイスブルー・白 |
...
- `evening`ではキャラクターおよび対象ピクセル画像にオレンジのオーバーレイを適用する
- 透明PNGの輪郭を自然に保てない場合は、夕方専用画像を用意して差し替える
- CSSで行う場合はラッパーの擬似要素または`filter`を使用し、クリック領域を妨げない
...
- 発動確率: 5%
- 発動時:
  - コンテンツ、ツール開発、アプリサービス、SNSのアイコンを食べ物PNGへ変更する
  - キャッチコピーを`おいしいごはんをいただきます`へ変更する
...
- 有効時間: 15:00〜15:30
- 発動確率: 5%
- 元の指定`15:00〜15:00`では時間幅がないため、実装上は30分枠として定義する
- 発動時:
  - 全体をはっきりしたピンクと白い水玉模様へ変更する
  - コンテンツ、ツール開発、アプリサービス、SNSのアイコンをお菓子PNGへ変更する
...
- 発動時:
  ...
  - 主要文字色を黒に変更する
  - 背景に目玉のドット絵を反復表示する
  - キャラクターと対象ドット絵を黒いシルエットにする
  - キャッチコピーを`はやく寝ろ`へ変更する
- 目玉画像は`/public/effects/eyes.png`に配置する
- シルエット化は`filter: brightness(0)`または専用黒画像を使用する
- フォーム入力欄は操作性維持のため白または薄い赤背景、黒文字、明確な枠線を維持する

### 抽選ルール

- 確率抽選は対象時間帯に入ったとき1回だけ行う
- ページを対象時間帯内に開いた場合は初期化時に1回抽選する
- 毎分再抽選しない
- 抽選結果は`localStorage`へ日付とイベント名を含むキーで保存し、同一ブラウザ・同一日では再読み込みや別タブでも再抽選しない

```text
yukimi-event:2026-07-10:lunch
yukimi-event:2026-07-10:snack
yukimi-event:2026-07-10:sleep-warning
```

- 保存値は`active`または`inactive`
- 日付が変わった場合は新しいキーで再抽選する
- `localStorage`が利用できない場合はメモリ上で一度だけ抽選する
...
開発環境で確率イベントを確認できるよう、クエリパラメータによる強制表示を許可する。
...
- 強制指定は`process.env.NODE_ENV !== 'production'`の場合のみ有効とする
- 本番環境ではクエリによる強制表示を無効化する
...
- 共通サイトフレーム内に表示する
- YukimiWorksの組織概要、会社情報、組織形態、経営理念を紹介する
- 内容ごとに独立したレトロパネルへ分割する

### 会社概要パネル

- タイトルバー: `会社概要 / Company`
- 本文最大幅は`800px`
- 短い紹介文は中央揃え、長文は左揃えとする

```text
YukimiWorksは、ユーザーに「良い」と思われるサービスやコンテンツを制作することを
目的として設立した組織です。

アプリや各種サービスの開発、イラスト制作などを中心に、日常の中で
「あれば便利」「あると嬉しい」と感じられるものを形にする活動を行っています。
```

### 会社情報パネル

- タイトルバー: `Company Data`
- タイトル列と内容列の2カラム表として表示する
- 各行の間を点線で区切る
- モバイルではタイトル列を上、内容を下にして1カラム化してよい

| タイトル | 内容 |
|---|---|
| 名称 | 合同会社YukimiWorks（ユキミワークス） |
| 所在地 | 神奈川県横浜市西区浅間町1丁目4番3号ウィザードビル402 |
| 代表 | 藤井 翔之 |
| 事業内容 | アプリ・サービス開発 / ゲーム制作 / デザイン制作 / デジタルコンテンツ制作 / イベント・セミナー企画 |
| 設立年月日 | 2026年3月4日 |

### 組織形態パネル

- タイトルバー: `組織形態 / Organization`
- 本文は左揃え、最大幅`800px`

```text
当社では、プログラマー、デザイナー、イラストレーター、ライバーなど、
それぞれの専門分野を持つ人材を社員という枠組みに限定せず、
「共同プロジェクトメンバー」として位置づけています。

個々が主体的に企画・制作に関わり、必要に応じてプロジェクト単位で協力しながら、
継続的にサービスやコンテンツを生み出していく体制を採用しています。

また当社は社長含め全員がクリエイターであり、成果を公平に分割するため、
固定給制度ではなく制作したコンテンツの収益をプロジェクトメンバー間で
分配する方式を採用しています。
```

### 経営理念パネル

- タイトルバー: `経営理念 / Philosophy`
- 「制作」「主体」「協力」「持続」の4カードを表示する
- デスクトップ4列、タブレット2列、モバイル1列
- 各キーワードは丸いグラデーションバッジではなく、角丸なしのピクセルラベルとしてカード上部に表示する
- カードは白背景、青系二重線風ボーダー、点線区切りを使用する

| キーワード | 説明 |
|---|---|
| 制作 | ユーザーにとって意味のあるものを作り続ける |
| 主体 | 自ら考え、自ら動く人が集まる |
| 協力 | 必要なときに力を合わせられる関係を築く |
| 持続 | 無理をせず、長く続けられる活動を行う |

### アニメーション

- 各パネルを短いフェードインで表示する
- カードは最大2〜4pxの移動に限定する
- `prefers-reduced-motion`ではフェードのみ、または即時表示とする
...
[data-theme='morning'] {
  --site-outer-bg: #e6f7ff;
  --site-bg: #f8fdff;
  --site-surface-soft: #edf9ff;
  --site-accent: #2877b7;
  --site-accent-soft: #8dd3f5;
  --site-text: #153e63;
  --site-border: #76bde4;
  --site-dotted: #a8d9ef;
}
...
| 中 | 昼食・お菓子・深夜警告イベント、アイコンセット切り替え、開発用強制表示 |
| 中 | Works分類導線、Philosophy、Pickup Works、News、Link、Contactのレトロパネル化 |
...
- 初回吹き出しがページ表示後約5秒で表示される
- 通常吹き出しが一定時間で消え、ランダム間隔後に再表示される
- キャラクタークリックで毎回画像が切り替わり、約2.5秒後に戻る
- 特殊メッセージの抽選範囲が排他的で、同時に複数表示されない
- 日本時間に基づいて6つの通常テーマが切り替わる
- 特殊イベントは対象時間帯ごと・日ごとに1回だけ抽選される
- `sleep-warning > snack > lunch > normal`の優先順位が守られる
...
*最終更新: 2026年7月 改訂6（Works統合・Portfolio／Link再編・キャリアプログラム廃止）*
