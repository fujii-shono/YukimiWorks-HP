...
- トップページへ`Philosophy`、`Pickup Works`、`News`、`Contact`など、サンプルに存在しない追加パネルを配置しない
- トップページのWelcome、Works、Linkの寸法、余白、タイトルバー、枠線、アイコン位置、文字位置を変更しない
...
- 本文は中央揃え
- 指定された改行を保持する
- 番号付き部分をブラウザ標準の`ol`へ変換して字下げや行間を変えず、指定文字列として表示する
...
- トップページの右カラムがWelcome、Works、Linkのみで構成される
...
- `/about`が指定文章、指定改行、中央揃え、代表者`ゆきみるく`で表示される
...
*最終更新: 2026年7月 改訂7（HTMLサンプル完全準拠・サブページ共通フレーム・開発用テーマテストUI・About内容更新）*
