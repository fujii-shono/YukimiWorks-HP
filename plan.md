...
- `DEBUG_OUTPUT_PRE_SMOOTHING_PATH` が `true` の場合も、`SHOW_LONG_STRAIGHT_LINE_MARKERS` が `true` ならスムージング前に検出済みの長い直線区間を青線として表示する。`SHOW_LONG_STRAIGHT_LINE_MARKERS` が `false` の場合は従来通り青線を出さない
