...
- SVGパスはサーバー生成マスクから境界を取り、外向き角はQ曲線、内向きの鋭角は直線角として残す
- SVGパス化では通常の大きな段差は残し、外向き角ごとに角度・辺長・深さから前後の接点を計算し、その点を制御点にしてQ曲線でつなぐ
- 一旦、細かなジグザグ補正や近距離点のC曲線化は外し、基本的なアウトライン生成に戻す
- 内側穴は境界追跡ではなく、計算した中心・半径から正円として出力する
